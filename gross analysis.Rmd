...
```{r}
genres.lasso.coef=predict(lasso.mod,type="coefficients",s=lasso.best.lam)[1:23,]
...
directors.co=data.frame(rownames(directors.co),directors.co)
library(dplyr)
...
```{r}
actors.lasso.coef=predict(lasso.mod,type="coefficients",s=lasso.best.lam)[1:1465,]
actors.lasso.coef=actors.lasso.coef[actors.lasso.coef!=0]
actors.lasso.coef
actors.co=data.frame(actors.lasso.coef)
actors.co=data.frame(rownames(actors.co),actors.co)
library(dplyr)
ordered.desc.actors=actors.co %>%
  arrange(desc(actors.lasso.coef))
ordered.desc.actors

ordered.asc.actors=actors.co %>%
