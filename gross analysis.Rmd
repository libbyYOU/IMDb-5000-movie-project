...
```{r}
library(dplyr)
data=select(movie,gross,budget,duration,director_facebook_likes, imdb_score, actor_1_facebook_likes, actor_2_facebook_likes,actor_3_facebook_likes, cast_total_facebook_likes, movie_facebook_likes, num_voted_users, num_critic_for_reviews, facenumber_in_poster)
...
```
```{r}
M3=lm(gross~.-facenumber_in_poster-imdb_score,train_data)
summary(M3)
```
```{r}
M4=lm(gross~.-facenumber_in_poster-imdb_score-movie_facebook_likes,train_data)
summary(M4)
```

```{r}
M5=lm(gross~.-facenumber_in_poster-movie_facebook_likes-imdb_score-num_critic_for_reviews,train_data)
summary(M5)
```
```{r}
M6=lm(gross~.-facenumber_in_poster-movie_facebook_likes-imdb_score-num_critic_for_reviews-duration,train_data)
summary(M6)
```
...
ridge.pre=predict(ridge.mod,s=best.lam,newx = x[-train,])
...
lasso.pre=predict(lasso.mod,s=best.lam,newx = x[-train,])
...
```{r}
genres.lasso.coef=predict(lasso.mod,type="coefficients",s=lasso.best.lam)
genres.lasso.coef
genres.lasso.coef[lasso.coef!=0]
```
...
```
```{r}
keywords.ridge.coef=predict(ridge.mod,type ="coefficients",s =ridge.best.lam )[1:5253,]
...
```{r}
keywords.lasso.coef=predict(lasso.mod,type="coefficients",s=lasso.best.lam)[1:5253,]
...
```{r}
directors.lasso.coef=predict(lasso.mod,type="coefficients",s=lasso.best.lam)[1:1465,]
directors.lasso.coef=directors.lasso.coef[directors.lasso.coef!=0]
directors.lasso.coef
directors.co=data.frame(directors.lasso.coef)
library(dplyr)
ordered.directors=directors.co %>%
